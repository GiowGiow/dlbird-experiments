...
class FocalLoss(nn.Module):
    ...
    def __init__(
        ...
        if gamma > 10.0:
            raise ValueError(
                f"gamma > 10.0 may cause numerical instability, got {gamma}"
            )
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        if inputs.size(0) != targets.size(0):
            raise ValueError(
                f"batch size mismatch: inputs {inputs.size(0)} vs targets {targets.size(0)}"
            )

        num_classes = inputs.size(1)
        if targets.min() < 0 or targets.max() >= num_classes:
            raise ValueError(
                f"targets must be in [0, {num_classes - 1}], got range [{targets.min()}, {targets.max()}]"
            )
