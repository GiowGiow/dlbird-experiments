...
class AudioSpectrogramDataset(Dataset):
    ...
    def __init__(
        ...
        missing_species = set(self.df["species_normalized"].unique()) - set(
            species_to_idx.keys()
        )
        if missing_species:
    ...
    def _extract_and_cache_spectrogram(self, idx: int) -> np.ndarray:
        ...
        try:
            ...
            expected_frames = (
                int((self.sr * self.target_duration - self.n_fft) / self.hop_length) + 1
            )
    ...
    def get_cache_stats(self) -> Dict[str, int]:
        ...
        cached_count = sum(
            1 for idx in range(len(self)) if self._get_cache_path(idx).exists()
        )
        return {
    ...
    for spec in spectrograms:
        if spec.shape[-1] < max_time:
            ...
            spec = torch.nn.functional.pad(
                spec, (0, pad_size), mode="constant", value=0
            )
