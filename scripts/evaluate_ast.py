...
def evaluate_model(args):
    ...
    with torch.no_grad():
        for batch_idx, (spectrograms, labels) in enumerate(dataloader):
            ...
            if (batch_idx + 1) % 50 == 0:
                print(
                    f"  Processed {(batch_idx + 1) * args.batch_size}/{len(dataset)} samples..."
                )
    ...
    print(
        f"Per-class results saved to {output_dir / f'{args.output_name}_per_class.json'}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output-name", type=str, required=True, help="Name for output files"
    )
    parser.add_argument(
