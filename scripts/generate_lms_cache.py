...
def main():
    ...
    parser.add_argument(
        "--force-regenerate", action="store_true", help="Regenerate all caches"
    )
    parser.add_argument(
        "--target-duration",
        type=float,
        default=4.0,
        help="Target audio duration (seconds)",
    )
    ...
    # Report errors
    errors = [
        r for r in results if r["status"] in ("error", "invalid", "missing_audio")
    ]
    if errors:
        print(f"\n⚠️  Errors ({len(errors)} total):")
        for err in errors[:10]:  # Show first 10
            print(
                f"   {err['xc_id']:15s} ({err['species']:30s}): {err.get('error', 'missing audio')}"
            )
        if len(errors) > 10:
    ...
    print(
        f"   Success rate: {total_ok:.1f}% ({status_counts.get('success', 0) + status_counts.get('cached', 0)}/{len(results)})"
    )
    print(f"{'=' * 80}")
